 ...
 * </div>
 * 手机一般都有麦克风和摄像头，而Android系统就可以利用这些硬件来录制 音视频 了。
 */
public class MediaRecorder
{
    ...
    public MediaRecorder()
    {
         ...
         */
        native_setup(new WeakReference<MediaRecorder>(this), packageName, ActivityThread
     ...
     *                               {@link #stop}, or is called when VideoSource is not set to
     *                               SURFACE.
    ...
    public void setProfile(CamcorderProfile profile)
    {
        ...
        if (profile.quality >= CamcorderProfile.QUALITY_TIME_LAPSE_LOW && profile.quality <=
     ...
     *            The fps can go as low as desired. However the fastest fps will be limited by
     *            the hardware.
     *            For resolutions that can be captured by the video camera, the fastest fps can
     *            be computed using
     ...
     *
     * @param max_duration_ms the maximum duration in ms (if zero or negative, disables the
     *                        duration limit)
     */
     ...
     * listener is notified.
     *
     * @param max_filesize_bytes the maximum filesize in bytes (if zero or negative, disables the
     *                           limit)
     */
    ...
    private native void _setOutputFile(FileDescriptor fd, long offset, long length) throws
